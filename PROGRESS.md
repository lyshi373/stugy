...
### 2026-08-13 README 全面重写

- 与当前项目状态对齐：在线地址、模块一览（含默认隐藏/密码状态）、底部导航、访问控制（密码与隐藏）、添加新软件、模块化结构与测试、发布流程、常见问题（数据记忆/缓存/密码）
- 移除过时的行号引用与失效入口说明

### 2026-08-13 强制重置默认隐藏显示状态

**问题**：部分浏览器因旧缓存或之前测试时点过「显示」，UG / DJI 仍显示在首页。

**改动**
- 新增 `DISPLAY_CFG_VERSION`：本次升级会清空一次旧的 `checkin_ui_show` 记录，确保默认隐藏重新生效（之后的「显示」选择仍会记住）
- 缓存版本升至 v17，强制旧客户端刷新
...
**改动**
- 新增管理密码 `ADMIN_PASSWORD`（默认 123456，位于 index.html 顶部配置区，可修改）
- 「👁 已隐藏」面板中「默认隐藏模块」点「显示」、以及「全部显示」按钮，都会先弹出密码框（`openAdminPwd`），密码正确才执行恢复
- 自己通过卡片隐藏的软件恢复不受影响（无需密码）
- 「如何新增」常见问题同步说明；缓存版本升至 v15
- `tools/test_app.js` 密码流程断言更新，51/51 通过

### 2026-08-13 默认隐藏模块（UG/DJI/SolidWorks）增加界面恢复入口

**需求**：此前默认隐藏只能改代码，需要一个界面入口来取消隐藏。

**改动**
- 左下角「👁 已隐藏」面板新增「🛠 默认隐藏模块（点击显示）」分区，列出 UG / DJI / SolidWorks，点「显示」即恢复（存入 `checkin_ui_show`，覆盖 `HIDDEN_SOFTWARE_IDS`）
- 「全部显示」按钮现在也会同时恢复默认隐藏模块
- `EXTENDED_SOFTWARE_IDS` 加入 solidworks，保证新设备首次打开也能在面板中找到它
- 「如何新增」常见问题更新：默认隐藏模块可自行恢复，不再需要联系管理员
- `tools/test_app.js` 新增 2 条断言（面板列出默认隐藏模块、显示后回到列表），49/49 通过

### 2026-08-13 隐藏软件界面（卡片隐藏 + 已隐藏面板）+ 缓存版本修复

**需求**：提供可视化的隐藏/恢复软件入口（此前只能改代码）。

**改动**
- 每个软件卡片新增「隐藏」按钮（内置与自定义都有），隐藏后不出现在首页与知识点列表
- 左下角新增「👁 已隐藏」悬浮按钮，打开面板可逐个「显示」或「全部显示」恢复
- 新增 `loadSoftwareRaw()`（未过滤列表）与 `loadSoftware()`（过滤代码级隐藏 + 界面隐藏 `checkin_hidden_ui`）
- 「如何新增」常见问题补充隐藏/恢复说明
- **修复缓存版本不一致**：index.html / template.html 的 `CACHE_VERSION` 由 v10 同步到 v13（此前 sw.js 已升到 v12，导致清理逻辑不触发、用户看到旧界面），sw.js 升至 v13
- `tools/test_app.js` 新增 3 条断言（UI 隐藏后不在列表、FAB 出现、恢复显示），47/47 通过
